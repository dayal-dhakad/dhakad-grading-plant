...
const input = z
  ...
  .parse({
    mobile: process.env.BOOTSTRAP_STAFF_MOBILE,
    name: process.env.BOOTSTRAP_STAFF_NAME,
...
try {
  ...
  const user = await prisma.user.create({
    data: { mobile: input.mobile, name: input.name, passwordHash, role: Role.STAFF },
  });
  console.log(`Staff user created: ${user.mobile}`);
} finally {
